...
const IngresarControlDocumentalPage = () => {
  const tiposDocumento = [
    { id: 'politicaCalidad', label: 'Política de Calidad Vigente' },
    { id: 'manualProcedimientos', label: 'Manual de Procedimientos Internos' },
    { id: 'registrosCapacitacion', label: 'Registros de Capacitación de Personal' },
    { id: 'informesNoConformidad', label: 'Informes de No Conformidad y Acciones Correctivas' },
    { id: 'auditoriaInterna', label: 'Resultados de Auditoría Interna de Procesos' }
  ];
  ...
  const handleSubmit = (event) => {
    ...
    if (documentosGuardar.length === 0) {
      alert('Por favor, seleccione al menos un documento de control.');
      return;
    }
  ...
  return (
    <div>
      ...
      <h2 className="mb-4 mt-3">Ingresar Archivos de Control Documental</h2>
            ...
            {tiposDocumento.map(doc => (
              <div className="form-check mb-3" key={doc.id}>
                <input
                  ...
                  type="checkbox"
                  name={doc.id}
                  id={`doc-control-${doc.id}`}
                  checked={!!documentosSeleccionados[doc.id]}
                  onChange={handleCheckboxChange}
                />
                <label className="form-check-label-neumorphic ms-2" htmlFor={`doc-control-${doc.id}`}>
                  {doc.label}
                </label>
              </div>
            ))}
...
export default IngresarControlDocumentalPage;
